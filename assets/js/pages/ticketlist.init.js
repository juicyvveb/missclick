...
var str_dt = function (e) {
    var e = new Date(e),
      ...
      a = "" + e.getDate(),
      e = e.getFullYear();
    return t.length < 2 && (t = "0" + t), [(a = a.length < 2 ? "0" + a : a) + " " + t, e].join(", ")
  },
  ...
  editlist = !1,
  options = {
    valueNames: ["id", "tasks_name", "client_name", "assignedto", "create_date", "due_date", "status", "priority"],
    page: perPage,
    pagination: !0,
...
}, xhttp.open("GET", "assets/json/support-tickets-list.json"), xhttp.send();
var isValue = (isCount = (new DOMParser).parseFromString(ticketsList.items.slice(-1)[0]._values.id, "text/html")).body.firstElementChild.innerHTML,
...
refreshCallbacks(), document.getElementById("showModal").addEventListener("show.bs.modal", function (e) {
  e.relatedTarget.classList.contains("edit-item-btn") ? (document.getElementById("exampleModalLabel").innerHTML = "Edit Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Update") : e.relatedTarget.classList.contains("add-btn") ? (document.getElementById("modal-id").style.display = "none", document.getElementById("exampleModalLabel").innerHTML = "Add Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "block", document.getElementById("add-btn").innerHTML = "Add Ticket") : (document.getElementById("exampleModalLabel").innerHTML = "List Ticket", document.getElementById("showModal").querySelector(".modal-footer").style.display = "none")
...
var count = 14,
  ...
  example = (Array.prototype.slice.call(forms).forEach(function (a) {
    a.addEventListener("submit", function (e) {
      var t;
      a.checkValidity() ? (e.preventDefault(), "" === tasksTitleField.value || "" === client_nameNameField.value || "" === assignedtoNameField.value || "" === dateField.value || "" === dateDueField.value || "" === statusField.value || "" === priorityField.value || editlist ? "" !== tasksTitleField.value && "" !== client_nameNameField.value && "" !== assignedtoNameField.value && "" !== dateField.value && "" !== dateDueField.value && "" !== statusField.value && "" !== priorityField.value && editlist && (t = ticketsList.get({
        id: idField.value
        ...
        order: "desc"
      }), document.getElementById("close-modal").click(), clearFields(), refreshCallbacks(), filterOrder("All"), count++, Swal.fire({
        position: "center",
        icon: "success",
  ...
  removeBtns && Array.from(removeBtns).forEach(function (e) {
    e.addEventListener("click", function (e) {
      ...
      Array.from(e).forEach(function (e) {
        ...
        deleteid.body.firstElementChild.innerHTML == itemId && document.getElementById("delete-record").addEventListener("click", function () {
          ticketsList.remove("id", t.outerHTML), document.getElementById("deleteRecord-close").click()
        })
      })
    })
    ...
    e.addEventListener("click", function (e) {
      ...
      Array.from(e).forEach(function (e) {
        ...
        t == itemId && (editlist = !0, idField.value = t, tasksTitleField.value = e._values.tasks_name, client_nameNameField.value = e._values.client_name, assignedtoNameField.value = e._values.assignedto, dateField.value = e._values.create_date, dateDueField.value = e._values.due_date, example && example.destroy(), example = new Choices(priorityField, {
          searchEnabled: !1
        }), t = (val = (new DOMParser).parseFromString(e._values.priority, "text/html")).body.firstElementChild.innerHTML, example.setChoiceByValue(t), statusVal && statusVal.destroy(), statusVal = new Choices(statusField, {
          searchEnabled: !1
...
function clearFields() {
  tasksTitleField.value = "", client_nameNameField.value = "", assignedtoNameField.value = "", dateField.value = "", dateDueField.value = "", example && example.destroy(), example = new Choices(priorityField), statusVal && statusVal.destroy(), statusVal = new Choices(statusField)
}

function deleteMultiple() {
  ids_array = [];
  var e, t = document.querySelectorAll(".form-check [value=option1]");
  for (i = 0; i < t.length; i++) 1 == t[i].checked && (e = t[i].parentNode.parentNode.parentNode.querySelector("td [data-id]").getAttribute("data-id"), ids_array.push(e));
  "undefined" != typeof ids_array && 0 < ids_array.length ? Swal.fire({
    ...
    if (e.value) {
      for (i = 0; i < ids_array.length; i++) ticketsList.remove("id", `<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="${ids_array[i]}" class="fw-medium link-primary ticket-id">#VLZ${ids_array[i]}</a>`);
...
function ViewTickets(e) {
  var e = e.getAttribute("data-id"),
    e = ticketsList.get("id", '<a href="javascript:void(0);" onclick="ViewTickets(this)" data-id="' + e + '" class="fw-medium link-primary ticket-id">#VLZ' + e + "</a>"),
    t = e[0]._values.id,
    a = document.createElement("div"),
    t = (a.innerHTML = t, a.innerText.slice(4));
  localStorage.setItem("ticket-list", JSON.stringify(e[0]._values)), localStorage.setItem("option", "view-ticket"), localStorage.setItem("ticket_no", t), window.location.assign("apps-tickets-details.html")
}
